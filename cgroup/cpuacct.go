...
func (c CPUAcct) UserSeconds() float64 {
	return float64(c.UsageUserNanosecs()) / float64(time.Second.Nanoseconds())
}

func (c CPUAcct) SystemSeconds() float64 {
	return float64(c.UsageSystemNanosecs()) / float64(time.Second.Nanoseconds())
}
